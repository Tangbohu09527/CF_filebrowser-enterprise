...
func TestMediaMetadataRequiresPreview(t *testing.T) {
	...
	file := fixture.files["full audio"]
	user := fixture.user(t, true, false, false)
	if !user.Permissions.Browse || user.Permissions.Preview || user.Permissions.Download {
		t.Fatalf("Browse-only metadata fixture permissions: %+v", user.Permissions)
	}

	response := fixture.metadataRequest(t, user, file, false)
	if response.status != http.StatusOK || response.err != nil {
		t.Fatalf("Browse-only metadata response: status=%d err=%v body=%q", response.status, response.err, response.body)
	}
	var decoded iteminfo.ExtendedFileInfo
	if err := json.Unmarshal(response.body, &decoded); err != nil {
		t.Fatalf("decode Browse-only metadata response: %v", err)
	}
	if decoded.Name != filepath.Base(file.realPath) || decoded.Path != file.indexPath || decoded.Type == "" || decoded.Size != int64(len(file.content)) {
		t.Errorf("Browse-only metadata omitted safe basic fields: %+v", decoded.FileInfo)
	}
	if decoded.Metadata != nil || len(decoded.Subtitles) != 0 || decoded.Content != "" {
		t.Errorf("Browse-only metadata exposed protected fields: metadata=%+v subtitles=%+v content=%q", decoded.Metadata, decoded.Subtitles, decoded.Content)
	}
	if bytes.Contains(response.body, file.content) {
		t.Errorf("Browse-only metadata exposed original file content %q", file.content)
	}
}
