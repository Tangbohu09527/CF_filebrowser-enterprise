...
def prepare(paths: Paths, args) -> None:
    ...
    if paths.metadata.exists():
        ...
        if any(existing.get(key) != value for key, value in {"source_sha": args.source_sha, "image_ref": args.image_ref, "mode": args.mode, "exposure": args.exposure}.items()):
    ...
    if args.exposure == "lan":
        ...
        config_value["server"]["allowedClientCIDRs"] = ["127.0.0.1/32"] + args.lan_allowed_cidrs.split(",")
    ...
    metadata = {"version": 1, "source_sha": args.source_sha, "image_ref": args.image_ref, "image_id": image["Id"], "mode": args.mode, "exposure": args.exposure, "test_disk": args.test_disk, "raid_confirmed": args.raid_confirmed, "bootstrap_complete": False, "admin_username": "admin"}
    if args.exposure == "lan":
